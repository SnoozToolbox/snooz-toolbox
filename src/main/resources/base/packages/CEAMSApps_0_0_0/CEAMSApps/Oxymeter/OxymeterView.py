...
from CEAMSApps.Oxymeter.ChannelSelectionDialog import ChannelSelectionDialog
from CEAMSApps.Oxymeter.ui.Ui_OxymeterView import Ui_OxymeterView

from CEAMSModules.PSGReader.PSGReaderManager import PSGReaderManager

class OxymeterView(Ui_OxymeterView, QtWidgets.QWidget):
    """
    """
    ...
    def remove_new_clicked(self):
        self.oxymeter_draw_area.exclusion_events = self._get_exclusion_events()
    ...
    def _save_file(self):
        ...
        if self._selected_oxymeter is None:
            log_msg = QMessageBox()
            log_msg.setWindowTitle("Information")
    ...
    def _close_file(self):
        ...
        self.oxymeter_draw_area.reset_exclusion_events()
        self.oxymeter_draw_area.set_signal_models([])
        self.oxymeter_draw_area.update()
    ...
    def _channel_selection_dialog(self):
        ...
        if dialog.exec_():
            if dialog.montage_selection == self._selected_montage and \
                dialog.oxymeter_selection == self._selected_oxymeter:
                return
            ...
            signal_models = psg_reader_manager.get_signal_models(self._selected_montage, [self._selected_oxymeter])
            psg_reader_manager.close_file()

            self._current_signal_models = []
            for signal_model in signal_models:
                if signal_model.channel == self._selected_oxymeter:
    ...
    def _get_exclusion_events(self):
        ...
        events = self._events[self._events['channels'] == self._selected_oxymeter]
