"""
@ Valorisation Recherche HSCM, Societe en Commandite – 2023
See the file LICENCE for full license details.
"""
"""
    SleepCycleDoc
    Step of the Tool Description
"""
import base64
from qtpy import QtWidgets
from qtpy.QtGui import QPixmap

from CEAMSTools.SleepCycleExport.SleepCycleDoc.Ui_SleepCycleDoc import Ui_SleepCycleDoc
from commons.BaseStepView import BaseStepView

class SleepCycleDoc( BaseStepView,  Ui_SleepCycleDoc, QtWidgets.QWidget):
    """
        SleepCycleDoc
        Step of the Tool Description
    """
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        # init UI
        self.setupUi(self)
        self._load_embedded_image()

    def load_settings(self):
        pass

    def on_apply_settings(self):
        pass

    def _load_embedded_image(self):
        """Load the embedded base64 image data into QLabel hypnogram_pic."""
        from .hyp_image_data import HYP_IMAGE_BASE64
        
        image_bytes = base64.b64decode(HYP_IMAGE_BASE64)
        pixmap = QPixmap()
        pixmap.loadFromData(image_bytes)
        self.hypnogram_pic.setPixmap(pixmap)