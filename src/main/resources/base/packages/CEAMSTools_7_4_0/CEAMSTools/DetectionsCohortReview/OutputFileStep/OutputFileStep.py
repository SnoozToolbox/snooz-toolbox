...
"""
from CEAMSTools.DetectionsCohortReview.InputFiles.InputFiles import InputFiles
from CEAMSTools.DetectionsCohortReview.OutputFileStep.Ui_OutputFileStep import Ui_OutputFileStep
...
class OutputFileStep(BaseStepView, Ui_OutputFileStep, QtWidgets.QWidget):
    ...
    def __init__(self, **kwargs):
        ...
        self._label_topic = f'{self._det_review_identifier}.activity_label'
        self._pub_sub_manager.subscribe(self, self._label_topic)               
        self._clean_topic = f'{self._det_review_identifier}.clean_flag'
        self._pub_sub_manager.subscribe(self, self._clean_topic)          
        self._transposed_topic = f'{self._det_review_identifier}.transposed_flag'
        self._pub_sub_manager.subscribe(self, self._transposed_topic)   
        self._output_topic = f'{self._det_review_identifier}.output_file'
        ...
        self.det_df = pd.DataFrame() # Will be updated when the context information is modified
    ...
    def on_topic_update(self, topic, message, sender):
        ...
        if topic == self._context_manager.topic:
            ...
            if message == InputFiles.context_InputFiles:
                ...
                # Look for detections on sleep stage
    ...
    def choose_button_slot(self):
        # Disable the overwrite message warning when selecting an existing file
    ...
    # Read the list of filenames and init self.det_df
