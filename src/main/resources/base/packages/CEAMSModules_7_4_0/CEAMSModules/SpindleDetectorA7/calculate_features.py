...
def sample_to_win(data_sample, win_length_sec, win_step_sec, sample_rate):
    ...
    strides = (n_samples_per_win_step*data_sample.strides[-1], data_sample.strides[-1])
    if verbose == 1:
        print("The strides are {} bytes\n"\
              "\t which is {} step samples per window x 8 bytes\n"\
              "\t and 1 sample x 8 bytes".format(strides, n_samples_per_win_step))
            
    # # Create a view into the array with the given shape and strides.
    # return np.lib.stride_tricks.as_strided(data1D, shape=shape,
    #                                           strides=strides, writeable=False)
    # Create a view into the array with the given shape and strides.
    return np.lib.stride_tricks.as_strided(data_sample, shape=shape, strides=strides)
...
def power_spectral_density(data, win_length_sec, win_step_sec, sample_rate, zero_pad_sec):
    ...
    fft_modules = np.abs(rfft(windows_scaled, n_samples_per_padded_fft_win))
...
def baseline_windows(data_per_win, win_length_sec, win_step_sec, bsl_length_sec):
    # The data_per_win matrix is n_windows x n_samples_per_win
    # The value of a sample is nan if the sample is marked as an artifact
...
# data_per_win is an array of n_windows x n_samples_per_win
def baseline_z_score(data_per_win, win_length_sec, win_step_sec, bsl_length_sec):
    ...
    # So the bsl for the first window is bsl_per_win[0,:] should cover 30 s bsl
    ...
    limits = np.nanpercentile(data_per_win, [10, 90])
    ...
    # select the indexes (axis=1) where at least 3 samples are not NaN
    # sum(axis=1) returns the number of non-NaN values per baseline
    z_score = np.full_like(data_per_win, np.nan)
    valid_bsl = (~np.isnan(bsl_per_win)).sum(axis=1) >= 3
    if any(valid_bsl):
        bsl_per_win_mean = np.nanmean(bsl_per_win[valid_bsl,:], axis=1)
        bsl_per_win_std = np.nanstd(bsl_per_win[valid_bsl,:], axis=1)     
        # Transform every sliding window value to its z-score using the corresponding baseline window
        z_score[valid_bsl] = (data_per_win[valid_bsl] - bsl_per_win_mean) / bsl_per_win_std
...
def relative_power_values(data, win_length_sec, win_step_sec, bsl_length_sec, sample_rate, frequency_band):
    ...
    relative_power_per_sample = win_to_sample(relative_power_per_win_z_score, win_length_sec, win_step_sec, sample_rate,
                                              n_samples)
    # # Make sure at least 3 samples have a value
    # if ~np.isnan(relative_power_per_sample).all(axis=1).sum() >= 3:
    #valid_sample = (~np.isnan(relative_power_per_sample)).sum(axis=0) >= 3
...
def covariance_values(data, win_length_sec, win_step_sec, bsl_length_sec, sample_rate, frequency_band):
    ...
    covariance_per_win_z_score = baseline_z_score(covariance_per_win_log10, \
        win_length_sec, win_step_sec, bsl_length_sec)

    # Calculate covariance per sample (multiple values for samples with overlapping windows)
    covariance_per_sample = win_to_sample(covariance_per_win_z_score, win_length_sec, \
        win_step_sec, sample_rate, n_samples)
...
def correlation_values(data, win_length_sec, win_step_sec, sample_rate, frequency_band):
    ...
    return np.nanmean(correlation_per_sample, axis=0)
