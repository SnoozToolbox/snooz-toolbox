...
"""
    The model of an epoch. Contains the epochs and its informations
...
class EpochModel:
    ...
    def clone(self, clone_samples=False):
        new_signal = EpochModel()

        if clone_samples:
            new_signal.samples = self.samples.copy()


        new_signal.num_epochs = self.num_epochs
    ...
    def get_attribute(signals, attr, group_by, value_to_test=None):
        """
        Get or Group in a list the desired attribute in the list of EpochModel
        * To use get_attribute(signals, 'samples', 'channel', 'c3') all the samples must have the same dimension.

        Parameters
        -----------
            signals : a list of EpochModel
            attr : The desired attribute. If None, will return the list of EpochModel (signals)
        ...
        #   ex) samples1 = EpochModel.get_attribute(signals, 'samples', None)
        ...
        # Group the list of dict (EpochModel) by the attribute group_by and convert it in an array
        #   ex) signals_events = EpochModel.get_attribute(signals, None, 'start_time')
        ...
        #   start_events = np.unique(EpochModel.get_attribute(signals, 'start_time', 'start_time'), axis=1)
        ...
        #   ex) fs_chan = EpochModel.get_attribute(signals, 'sample_rate', 'chan_label', channel)
        else:
    ...
    def is_vectorizable(signals, same_events):
        ...
        -----------
            signals : A list of EpochModel
            same_events : Bool if all EpochModel in list should be from a same event
