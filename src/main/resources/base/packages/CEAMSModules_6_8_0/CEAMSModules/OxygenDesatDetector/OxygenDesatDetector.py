...
class OxygenDesatDetector(SciNode):
    ...
    def compute(self, artifact_group, artifact_name, arousal_group, arousal_name, \
        ...
        out_columns = list(_get_doc(self.N_CYCLE, self.stage_stats_labels, self.values_below).keys())
        ...
        if write_header:
            ...
            if not os.path.exists(doc_filepath):
                write_doc_file(doc_filepath, self.N_CYCLE, self.stage_stats_labels, self.values_below)
    ...
    def compute_cycles_stats_saturation(self, sleep_cycles_df, signals):
        ...
        """    
        cyc_dict = {}
        for i_cyc in range(self.N_CYCLE):
