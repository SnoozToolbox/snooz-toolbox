...
class EventsProxyModel(QtCore.QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, sourceRow, sourceParent):
        ...
        if ((item is not None) and (item.parent() is not None)):
            ...
            if self.name_search_pattern is not None:
                return self.name_search_pattern in cur_text
            else:
                return True
        # filename
        else:
    ...
    def set_names_search_pattern(self, pattern):
        self.name_search_pattern = pattern
        self.invalidate()
